...
`endif
    ...
    logic [5:0] load_idx;  // Progress in terms of beats (each beat = BANKING_FACTOR elements)
    
    // Address offset: each load_idx corresponds to BANKING_FACTOR elements
    wire [ADDRESS_WIDTH-1:0] load_idx_addr =
        {{(ADDRESS_WIDTH-$bits(load_idx)-3){1'b0}}, load_idx, 3'b000}; // Multiplied by 8 because BANKING_FACTOR=8
