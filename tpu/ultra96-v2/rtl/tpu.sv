...
// Instantiation of Axi Bus Interface M00_AXIS
	...
	//DMA to bram wires
	wire [12:0] dma_addr;
	wire [255:0] dma_dram_din, dma_dout;
    ...
    // BRAM to compute interface wires (port b) - 8 banks
    wire [12:0] comp_addr_b [8];
    wire [31:0] comp_din_b  [8];
    wire [31:0] comp_dout_b [8];
    wire        comp_en_b   [8];
    wire        comp_we_b   [8];
    ...
    //  Instantiate 4 scratchpad banks
    generate
    for (genvar i = 0; i < 8; i++) begin : gen_scratchpad
            scratchpad #(
                .ADDR_WIDTH(13),
                .DATA_WIDTH(32)
            ) u_scratchpad (
                ...
                // DMA AXI-Stream interfaces - each bank gets 32 bits of the 128-bit bus
                .dma_wr_en(data_write_en),
                .dma_wr_data(dma_dram_din[i*32 +: 32]),
                .dma_write_pointer(write_pointer),
                
                .dma_rd_en(read_en),
                .dma_rd_data(dma_dout[i*32 +: 32]),
                .dma_read_pointer(read_pointer),
                
                // Compute-side BRAM port
                .dma_comp_addr_b(comp_addr_b[i]),
                .dma_comp_din_b (comp_din_b[i]),
                .dma_comp_dout_b(comp_dout_b[i]),
                .dma_comp_en_b  (comp_en_b[i]),
                .dma_comp_we_b  (comp_we_b[i])
            );
        end
    endgenerate
    ...
    .VPU_IADDR_W(5)
        ...
        // 8-bank memory interface
        .bram_addr_b0(comp_addr_b[0]), .bram_din_b0(comp_din_b[0]), .bram_dout_b0(comp_dout_b[0]), .bram_en_b0(comp_en_b[0]), .bram_we_b0(comp_we_b[0]),
        .bram_addr_b1(comp_addr_b[1]), .bram_din_b1(comp_din_b[1]), .bram_dout_b1(comp_dout_b[1]), .bram_en_b1(comp_en_b[1]), .bram_we_b1(comp_we_b[1]),
        .bram_addr_b2(comp_addr_b[2]), .bram_din_b2(comp_din_b[2]), .bram_dout_b2(comp_dout_b[2]), .bram_en_b2(comp_en_b[2]), .bram_we_b2(comp_we_b[2]),
        .bram_addr_b3(comp_addr_b[3]), .bram_din_b3(comp_din_b[3]), .bram_dout_b3(comp_dout_b[3]), .bram_en_b3(comp_en_b[3]), .bram_we_b3(comp_we_b[3]),
        .bram_addr_b4(comp_addr_b[4]), .bram_din_b4(comp_din_b[4]), .bram_dout_b4(comp_dout_b[4]), .bram_en_b4(comp_en_b[4]), .bram_we_b4(comp_we_b[4]),
        .bram_addr_b5(comp_addr_b[5]), .bram_din_b5(comp_din_b[5]), .bram_dout_b5(comp_dout_b[5]), .bram_en_b5(comp_en_b[5]), .bram_we_b5(comp_we_b[5]),
        .bram_addr_b6(comp_addr_b[6]), .bram_din_b6(comp_din_b[6]), .bram_dout_b6(comp_dout_b[6]), .bram_en_b6(comp_en_b[6]), .bram_we_b6(comp_we_b[6]),
        .bram_addr_b7(comp_addr_b[7]), .bram_din_b7(comp_din_b[7]), .bram_dout_b7(comp_dout_b[7]), .bram_en_b7(comp_en_b[7]), .bram_we_b7(comp_we_b[7])
    );
