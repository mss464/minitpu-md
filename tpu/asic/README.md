...
**Objective**: Baseline digital implementation for `tensorcore` targeting 50 MHz.
...
| Aspect | FPGA (`tpu.sv`) | ASIC (`tensorcore.sv`) |
|--------|------------------------|----------------------|
...
| **Location** | `tpu/tpu.sv` | `asic/src/tensorcore.sv` |

```
FPGA System:                              ASIC System:
┌──────────┐    ┌─────────────┐          ┌──────────┐
│ Xilinx   │───►│ tpu  │          │ SPI/GPIO │───► tensorcore
│ AXI DMA  │    │ (AXI-Stream)│          │ Bridge   │     (valid/ready)
└──────────┘    └─────────────┘          └──────────┘
```

Both wrappers instantiate the **same internal modules**: `compute_core`, `scratchpad`, `systolic`, `mxu`, `vpu`, etc.
...
6.  **GDS Generation**: ✅ Completed in 12s.
    *   Final layout generated in `results/ihp-sg13g2/tensorcore/base/6_final.gds`.

### ASIC-Specific Modifications
To enable the flow, the following files were modified/created in `asic/src/`:
*   `tensorcore.sv`: Top-level wrapper stripping AXI/Xilinx IPs.
*   `scratchpad.sv`: Fixed non-constant initialization.
...
*   `mxu.sv`: Removed unsupported `function automatic`.
...
docker run --rm -v $(pwd):/design -w /OpenROAD-flow-scripts/flow openroad/orfs:latest bash -c "
    # Setup directories
    mkdir -p designs/src/tensorcore designs/ihp-sg13g2/tensorcore

    # Copy RTL (Base + ASIC fixes)
    cp /design/tpu/*.sv designs/src/tensorcore/
    cp /design/asic/src/*.sv designs/src/tensorcore/
    
    # Copy Config
    cp /design/asic/ihp-sg13g2/config.mk designs/ihp-sg13g2/tensorcore/
    cp /design/asic/ihp-sg13g2/constraint.sdc designs/ihp-sg13g2/tensorcore/

    # Run complete flow
    source ../env.sh
    make finish DESIGN_CONFIG=designs/ihp-sg13g2/tensorcore/config.mk
"
```
